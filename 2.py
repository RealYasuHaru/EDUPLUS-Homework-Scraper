...
def main(session_cookie, hm_lvt_cookie, course_id):
    # 创建目录结构
    base_dir = os.getcwd()
    ...
    print(f"SESSION Cookie: {session_cookie[:6]}...{session_cookie[-6:]}")
    print(f"Hm_lvt Cookie: {hm_lvt_cookie}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--session', type=str, default=DEFAULT_SESSION_COOKIE,
                        help=f'SESSION cookie值 (默认: {DEFAULT_SESSION_COOKIE[:6]}...{DEFAULT_SESSION_COOKIE[-6:]})')
    parser.add_argument('--hm_lvt', type=str, default=DEFAULT_HM_LVT_COOKIE,
                        help=f'Hm_lvt cookie值 (默认: {DEFAULT_HM_LVT_COOKIE})')
    parser.add_argument('--course_id', type=str, default=DEFAULT_COURSE_ID,
                        help=f'课程ID (默认: {DEFAULT_COURSE_ID})')
